...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        temp_def = other_card.defense
        other_card.defense = other_card.attack
        other_card.attack = temp_def
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        def fitler_cards(card):
            return (
                card.attack != other_card.attack and card.defense != other_card.defense
            )

        new_opponent_cards = list(filter(fitler_cards, opponent.deck.cards))
        opponent.deck.cards = new_opponent_cards
