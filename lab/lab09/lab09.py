...
def make_advanced_counter_maker():
    ...
    def make_personal_counter():
        personally_counter = 0

        def count(type):
            nonlocal personally_counter
            ...
            if type == "count":
                personally_counter += 1
                return personally_counter
            if type == "reset":
                personally_counter = 0
...
def insert(link, value, index):
    ...
    if index == 0 and link is not Link.empty:
        next_link = Link(link.first)
        link.first = value
        next_link.rest = link.rest
        link.rest = next_link
    elif index == 0 and link is Link.empty:
        raise IndexError
    else:
