...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        call_func = self.operator.eval(env)
        params = [param.eval(env) for param in self.operands]
        return call_func.apply(params)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        current_env = dict(zip(self.parameters, arguments))
        current_env = dict(self.parent, **current_env)
