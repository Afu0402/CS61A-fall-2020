...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        while opponent_score:
            current_fn = digit_fn(opponent_score % 10)
            score = current_fn(score)
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    score = 0
    if num_rolls == 0:
        score = hefty_hogs(player_score, opponent_score)
    else:
...
def play(
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            current_player = score0
            current_strat = strategy0
            opponent = score1
        else:
            current_player = score1
            current_strat = strategy1
            opponent = score0

        roll_num = current_strat(current_player, opponent)
        score = take_turn(roll_num, current_player, opponent, dice, goal)
        current_player += score
        current_player += hog_pile(current_player, opponent)

        if who == 0:
            score0 = current_player
        else:
            score1 = current_player
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    message = f"Player {current_leader} takes the lead by {diff}"
    return current_leader, message
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_function(*argv):
        res = 0
        i = 0
        while i < total_samples:
            res += original_function(*argv)
            i += 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    get_average_total = make_averaged(roll_dice, total_samples)
    i = 1
    max_roll_num = 1
    current_max_average_score = 0
    while i <= 10:
        res = get_average_total(i, dice)
        if res > current_max_average_score:
            current_max_average_score = res
            max_roll_num = i
        i += 1
    return max_roll_num
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if hog_pile(score + hefty_hogs(score, opponent_score), opponent_score) > 0:
        return 0
