...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not current_place is beehive:
            if i >= self.min_range and i <= self.max_range and current_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            damage += self.damage
        for i in list(range(len(bees))):
            bees[i].reduce_armor(damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    implemented = True  # Change to True to view in the GUI
    # END Problem EC
    already_have_queen = False

    def __init__(self, armo=1):
        # BEGIN Problem ECr
        "*** YOUR CODE HERE ***"
        ScubaThrower.__init__(self, armo)
        if not QueenAnt.already_have_queen:
            QueenAnt.already_have_queen = True
