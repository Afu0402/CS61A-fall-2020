...
def about(topic):
    ...
    def select(paragragh):
        paragraghs = split(remove_punctuation(lower(paragragh)))
        for t in topic:
            if t in paragraghs:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    res = 0
    length = min(typed_length, reference_length)
    for i in range(length):
        if typed_words[i] == reference_words[i]:
            res += 1
    return res / typed_length * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff_amounts = [diff_function(typed_word, word, limit) for word in word_list]
    min_diff = min(diff_amounts)
    print("DEBUG", diff_amounts)
    if min_diff <= limit:
        for i in range(len(diff_amounts)):
            if diff_amounts[i] == min_diff:
                return word_list[i]
    else:
        return typed_word
    # END PROBLEM 5
...
def time_per_word(words, times_per_player):
    ...
    def make_time_difference(timestamp):
        if len(timestamp) == 1:
            return []
        return [timestamp[1] - timestamp[0]] + make_time_difference(timestamp[1:])

    time_arr = [make_time_difference(sub_arr) for sub_arr in times_per_player]

    return match(words, time_arr)
...
def fastest_words(match):
    ...
    for i in word_indices:
        player_times = [time(match, k, i) for k in player_indices]
        res[player_times.index(min(player_times))].append(word_at(match, i))
    return res
